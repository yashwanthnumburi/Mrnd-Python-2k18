...
def test_function_call_with_keyword_arguments():
    ...
    assert [10, 20, 30, 4] == demo(first=10,second=20, third=30)
    assert [10, 20, 30, 4] == demo(first=10,second=20,third=30)
    assert [10, 20, 3, 4] == demo(second=20,first=10)  # is this allowed? correct and uncomment
...
def simple_format(format, *args):
    ...
    for i in range(0,len(format)):
        if format[i]=='%':
            ...
            if format[i]>='0' and format[i]<='9' and int(format[i])<len(args):
                pass
            else:
                st=st+format[i]
...
-I have learnt what are parameters what are aruguments
-I have come to know what are named and positional argumrnts
-I learnt how to unpack and came to know what named arguments return and positional arguments return
-args is tuple type,kwargs is dict type
-we need to send *args and **kwargs rather than args and kwargs to other functions if not all will be treated as args
which includes a tuple and a dictionary .
"""
