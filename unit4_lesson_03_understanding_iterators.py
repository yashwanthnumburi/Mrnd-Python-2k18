...
def test_enumerate_iter():
    ...
    try:
        ...
        assert   None== list_iter.__next__() #note what happens when items are finished.
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['all','any','enumerate','filter','min','max','next',"sorted","sum","tuple","zip","join"]
    assert 12 == len(funcs)
...
-I have learnt what are iterators and how list,tuple,dict and set works using iterators
-iter function creates a iterator to the iterable item and once iterator moves right it cannot move back or left
-accessing elements can be done by using object.__next__()
-and  the exception here we have to use is StopIteration
-iter function returns TypeError if parameter passed to it is not iterable such as int
"""