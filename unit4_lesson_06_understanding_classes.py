...
def test_classes_are_object_factories():
    ...
    q2 = TestQueue()
    assert "<class 'TestQueue'>" == q1.__class__
    assert "<class 'TestQueue'>" == q2.__class__
...
def test_classes_bound_and_unbound_methods():
    ...
    assert '<bound method test_classes_bound_and_unbound_methods.<locals>.TestQueue.push of <TestQueue object at 0x05AA1B50>>' == TestQueue.push.__self__   #unbound method
    assert '<function test_classes_bound_and_unbound_methods.<locals>.TestQueue.push at 0x05ABCDB0>' == q1_push.__self__      #bound method, is associated with an instance
...
-I have learnt how to create classes in python.
-I have came to know that whenever an object is created for a class __init__() method is invoked.
-Similar to this pointer in  java in python we have self.
-It is bad to change the value inside the __init__ function.
-Its better to just initialize objects in __init__ function.
"""